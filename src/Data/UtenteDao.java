...
public class UtenteDao {
    ...
    private PreparedStatement classPerGioco ;
    private ResultSet rs;
        ...
        public UtenteDao() throws SQLException{
            ...
            iUtente = cn.conn.prepareStatement  ("insert into utente(username,nome,cognome,password,indirizzo,email,città) value (?,?,?,?,?,?,?)");
            checkUtente = cn.conn.prepareStatement("SELECT * FROM utente WHERE username = ? and password = ?");
            classPerGioco = cn.conn.prepareStatement("SELECT  * from partita where titologiocopartita = ? order by punteggio desc ");
        }
        ...
        public List<Utente> classificaPerPunteggioGioco(String nomegioco) throws SQLException{
        List<Utente> lista = new ArrayList();
        
        classPerGioco.setString(1, nomegioco);
        
        rs = classPerGioco.executeQuery();
        
        while (rs.next()){
        
            Utente user = new Utente();
            user.setUsername(rs.getString("usernamepartita"));
        ...
        return  lista ;
         ...
         public List<Utente> classificaPerPunteggioSystem(){
        List<Utente> lista = new ArrayList();
